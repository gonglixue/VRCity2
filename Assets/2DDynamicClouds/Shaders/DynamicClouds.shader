Shader "Clouds/2D Dynamic Clouds" {
	Properties{
		...
		_SkyBackground("Sky background texture 1", 2D) = "white"{}
		_SkyBackground2("Sky background texture 2", 2D) = "white"{}
		_SkyBackground3("Sky background texture 3", 2D) = "white"{}
		...
		_Lightning("Lightning", Range(0.0, 3.0)) = 0.0
	}
	SubShader{
		...
		Pass{
			...
			sampler2D _SkyBackground2;
			sampler2D _SkyBackground3;
			fixed _Lightning;
			...
			fixed4 frag(v2f i) : SV_Target{
				...
				else if (_Lightning >= 0.5 && _Lightning < 1.1) // _Lightning 1.0
					col.rgb = tex2D(_SkyBackground, i.uv).rgb*0.8 + col.rgb;
				else if (_Lightning > 1.5 && _Lightning < 2.1)
					col.rgb = tex2D(_SkyBackground2, i.uv).rgb*0.8 + col.rgb;
				else if (_Lightning > 2.5)
					col.rgb = tex2D(_SkyBackground3, i.uv).rgb * 0.8 + col.rgb;
